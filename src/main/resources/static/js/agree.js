...
function emailConfirmAlert(parentTag) {
    ...
    emailConfirmInput.addEventListener('focusout', function () {
        ...
        if (emailConfirmInput.value === '') {
            ...
            emailConfirmError.textContent = '이메일 주소를 입력해주세요.'
        } else if (commonEmailReg.test(emailConfirmInput.value) === false) {
            emailConfirmError.style.display = 'block'
            emailConfirmError.textContent = '올바른 이메일 주소가 아닙니다.'
        } else {
            ...
            emailConfirmBtn.addEventListener('click', function () {
                ...
                fetch(`${baseUrl}/join-confirm`, {
                    ...
                    emailConfirmStatus = response.status
                })
            })

            if (emailConfirmStatus === 201) {
                emailConfirmTitle.textContent = '메일의 인증번호를 입력하세요.'
                emailConfirmBtn.setAttribute('value', '인증하기')

                emailConfirmInput.addEventListener('focusout', function () {
                    if (emailConfirmInput.value === '') {
                        emailConfirmError.style.display = 'block'
                        emailConfirmError.textContent = '토큰번호를 입력해주세요.'
                    } else if (commonTokenReg.test(emailConfirmInput.value)) {
                        emailConfirmError.style.display = 'block'
                        emailConfirmError.textContent = '숫자로 된 6자리 토큰번호를 입력해주세요.'
                    } else {
                        emailConfirmError.style.display = 'none'
                        emailConfirmBtn.addEventListener("click", function () {
                            fetch(`${baseUrl}/join-confirm/${emailConfirmInput.value}`, {
                                ...
                                if (response.status === 200) {
                                    ...
                                    alert('토큰번호가 일치하지 않습니다. 이 전 화면으로 돌아갑니다.')
